...
<nav class="sidebar" id="sidebar">
  ...
  <div class="sidebar-nav">
    ...
    <div class="nav-section" style="margin-top:14px">Categorías</div>
...
<script>
// ══════════════════════════════════════════════════════════════
// HomeStock AI — Frontend (Flask version)
// Toda la lógica de negocio vive en app.py
// Este script solo maneja UI y llama a la API REST
// ══════════════════════════════════════════════════════════════
...
// ── Estado global ──────────────────────────────────────────────
let DATA = { products: [], mercadoHistory: [] };
...
async function api(method, path, body) {
  ...
  if (!res.ok) { showToast('Error: ' + res.status); return null; }
  return res.json();
}

async function loadProducts() {
  const ps = await api('GET', '/products');
  if (ps) DATA.products = ps;
}

// ── Helpers de display (usan campos _rate, _daysLeft, _status del server) ──
function recalcRate(p)   { return p._rate     ?? null; }
function getDaysLeft(p)  { return p._daysLeft  ?? null; }
function getStatus(p)    { return p._status    ?? 'unknown'; }
function getLastOpen(p)  { return p._hasOpen   ? true : null; }

function statusLabel(s, d) {
  if (s==='unknown') return 'Sin datos';
  if (s==='critical') return d < 1 ? '¡Agotado!' : `${Math.round(d)}d crítico`;
  return `${Math.round(d)}d quedan`;
}
function fmtDate(d) {
  if (!d) return '—';
...
function fmtN(n, u) {
  ...
  return (u==='und'||u==='pastillas') ? `${Math.round(n)}` : n<10 ? n.toFixed(1) : Math.round(n).toLocaleString('es-CO');
}
...
function updateBadges() {
  CATEGORIES.forEach(cat => {
    ...
    if (el) {
      el.textContent = ps.length;
      el.style.background = alerts > 0 ? 'rgba(220,38,38,.3)' : '';
      el.style.color      = alerts > 0 ? '#fca5a5' : '';
    }
  });
}
...
async function render() {
  ...
  else if (currentView==='graficas') { m.innerHTML = renderGraficas(); setTimeout(drawGraficas, 80); }
...
function renderDashboard() {
  ...
  const crit  = wData.filter(p => getStatus(p) === 'critical');
  const warn  = wData.filter(p => getStatus(p) === 'warning');
  const ok    = wData.filter(p => getStatus(p) === 'ok');
  ...
  return `
    ...
    ${alerts.length ? `<div class="alert-strip">
      ...
      <p><strong>${alerts.length} producto${alerts.length>1?'s':''}</strong> necesitan atención: ${alerts.map(p=>p.name).join(', ')}</p>
      <span class="alert-count">${alerts.length}</span>
    </div>` : ''}

    <div class="summary-cards">
      <div class="summary-card"><div class="label">TOTAL</div><div class="value">${all.length}</div><div class="sub">${CATEGORIES.length} categorías</div></div>
      <div class="summary-card good"><div class="label">OK</div><div class="value">${ok.length}</div><div class="sub">+7 días de stock</div></div>
      <div class="summary-card warning"><div class="label">ATENCIÓN</div><div class="value">${warn.length}</div><div class="sub">menos de 7 días</div></div>
      <div class="summary-card danger"><div class="label">CRÍTICOS</div><div class="value">${crit.length}</div><div class="sub">menos de 3 días</div></div>
    </div>

    <div id="lista-mercado-container"><p style="color:var(--text-muted);font-size:13px;padding:8px 0">Cargando lista de mercado...</p></div>
    ...
    <div class="toolbar">
      ...
      <div class="filter-tabs">
        ...
        <button class="filter-tab" onclick="setFilter('critical',this)">Críticos</button>
        <button class="filter-tab" onclick="setFilter('warning',this)">Atención</button>
    ...
    <div class="products-grid" id="products-grid">
      ${all.filter(p => p._hasOpen).map(p => renderCard(p)).join('')}
      ${all.filter(p=>p._hasOpen).length===0 ? '<div style="grid-column:1/-1;text-align:center;padding:48px;color:var(--text-muted)"><p>No hay productos abiertos. Registra un mercado para empezar.</p></div>' : ''}
    </div>`;
}

// Carga la lista de mercado desde la API después de renderizar
async function loadListaMercado() {
  const lista = await api('GET', '/lista-mercado');
  const el = document.getElementById('lista-mercado-container');
  if (!el || !lista) return;
  el.innerHTML = renderListaMercadoHTML(lista);
}

function renderListaMercadoHTML(lista) {
  if (!lista.length) return `<div style="background:var(--teal-light);border:1.5px solid var(--teal);border-radius:var(--radius);padding:14px 18px;margin-bottom:20px;display:flex;align-items:center;gap:10px">
    <span>🎉</span><div><div style="font-weight:600;color:var(--teal-dark);font-size:13px">¡Todo en orden!</div>
    <div style="font-size:12px;color:var(--teal-dark);opacity:.8">Ningún producto se termina en los próximos 14 días.</div></div></div>`;
  ...
  return `<div style="background:var(--surface);border:1.5px solid var(--border);border-radius:var(--radius);padding:18px 20px;margin-bottom:20px;box-shadow:var(--shadow)">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:8px">
      <div style="display:flex;align-items:center;gap:8px">
        <span>🛒</span>
        <div><div style="font-weight:600;font-size:14px">Lista para el próximo mercado</div>
        <div style="font-size:11.5px;color:var(--text-muted)">${lista.length} producto${lista.length>1?'s':''} en los próximos 14 días</div></div>
      </div>
      <button class="btn btn-primary btn-sm" onclick="setView('mercado')">Ir a registrar mercado</button>
    </div>
    <div style="display:flex;flex-direction:column;gap:6px">
      ${lista.map(item => {
        const dotColor = item.status==='critical'?'var(--red)':item.status==='warning'?'var(--amber)':'var(--gray)';
        const badge = item.daysLeft !== null
          ? `<span style="font-size:11px;font-weight:600;color:${dotColor};background:${item.status==='critical'?'var(--red-light)':item.status==='warning'?'var(--amber-light)':'var(--gray-light)'};padding:2px 8px;border-radius:20px">${Math.round(item.daysLeft)}d</span>`
          : `<span style="font-size:11px;color:var(--gray);background:var(--gray-light);padding:2px 8px;border-radius:20px">sin stock</span>`;
        const hint = item.needed
          ? `<span style="font-size:11px;color:var(--text-muted)">~${item.needed} ${item.unit}</span>`
        ...
        return `<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:var(--bg);border-radius:8px;gap:8px;flex-wrap:wrap">
          <div style="display:flex;align-items:center;gap:8px">
            ...
            <span style="font-size:11px;color:var(--text-muted)">${item.category}</span>
          </div>
...
function renderCard(p){
  const d=getDaysLeft(p), s=getStatus(p), rate=recalcRate(p);
  let pct=0, fillCls='fill-ok';
  if(d!==null&&p._hasOpen){
    const last=p.entries.filter(e=>e.opened&&!e.closed).sort((a,b)=>new Date(b.opened)-new Date(a.opened))[0];
    if(last){ const max=last.qty/(rate||1); pct=Math.min(100,Math.max(0,(d/max)*100)); fillCls=s==='critical'?'fill-critical':s==='warning'?'fill-warning':'fill-ok'; }
  }
  ...
  return `<div class="product-card status-${s}" data-pid="${p.id}" onclick="openDetail('${p.id}')">
    ...
    ${p._hasOpen?`<div style="margin-top:10px;display:flex;gap:6px" onclick="event.stopPropagation()">
      <button class="btn btn-sm" style="background:var(--red-light);color:var(--red);font-size:11px;padding:5px 10px" onclick="quickClose('${p.id}')">✓ Terminé esto</button>
    </div>`:''}
  </div>`;
}
...
function renderDetailModal(pid){
  ...
  const rate=recalcRate(p), d=getDaysLeft(p), s=getStatus(p);
  const pillCls={critical:'pill-critical',warning:'pill-warning',ok:'pill-ok',unknown:'pill-unknown'}[s];
  ...
  document.getElementById('modal-body').innerHTML=`
    ...
    <div class="detail-tabs">
      ...
      <button class="detail-tab ${detailTab==='graficos'?'active':''}" onclick="switchTab('${pid}','graficos')">Gráficos</button>
    </div>
  ...
  return `
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:18px">
      <div style="background:var(--bg);border-radius:10px;padding:12px;text-align:center">
        ...
        <div style="font-size:10.5px;color:var(--text-muted);margin-top:2px">${p.unit}/día</div>
      </div>
      <div style="background:var(--bg);border-radius:10px;padding:12px;text-align:center">
        ...
        <div style="font-size:10.5px;color:var(--text-muted);margin-top:2px">días restantes</div>
      </div>
      <div style="background:var(--bg);border-radius:10px;padding:12px;text-align:center">
    ...
    ${p.price!==null&&p.price!==undefined?`<div style="background:var(--teal-light);border-radius:10px;padding:11px 14px;margin-bottom:14px;display:flex;align-items:center;justify-content:space-between">
...
function detailHistorial(p){
  ...
  if(!sorted.length) return '<div style="text-align:center;padding:30px;color:var(--text-muted);font-size:13px">Sin entradas todavía.<br><br><button class="btn btn-primary btn-sm" onclick="openAddEntry(''+p.id+'')">+ Agregar primera entrada</button></div>';
  return `<div style="overflow-x:auto"><table class="history-table"><thead><tr>
    <th>Cantidad</th><th>Abierto</th><th>Cerrado</th><th>Días</th><th>Tasa</th><th>Precio</th><th></th>
  </tr></thead><tbody>
    ${sorted.map(e=>{
...
function detailGraficos(p){
  return `<div class="chart-wrap"><div class="chart-title">TASA DE CONSUMO (${p.unit}/día)</div><canvas id="chart-rate" height="150"></canvas></div>
    <div class="chart-wrap"><div class="chart-title">DÍAS POR PRESENTACIÓN</div><canvas id="chart-days" height="150"></canvas></div>`;
}
...
function drawProductCharts(p){
  ...
  const rc=document.getElementById('chart-rate'); if(rc) new Chart(rc,cfg(rates,p.unit+'/día','#1A7B6E'));
  const dc=document.getElementById('chart-days'); if(dc) new Chart(dc,cfg(days,'Días','#D97706'));
}
...
function openAddEntry(pid){
  ...
  document.getElementById('modal-body').innerHTML=`
    <p style="font-size:12.5px;color:var(--text-muted);margin-bottom:18px">Registra una compra de <strong>${p.name}</strong>.</p>
    <div class="form-grid">
      ...
      <div class="form-group"><label class="form-label">FECHA CIERRE (si ya terminó)</label><input class="form-input" type="date" id="e-closed"></div>
    </div>
...
async function saveEntry(pid){
  ...
  const result = await api('POST', `/products/${pid}/entries`, {qty,opened,closed,price});
  if(!result) return;
  // Update local product
  const idx=DATA.products.findIndex(x=>x.id===pid);
  if(idx>=0) DATA.products[idx]=result;
  closeModal(); render(); showToast(`Entrada guardada ✓`);
}

async function deleteEntry(pid,eid){
  const result = await api('DELETE', `/products/${pid}/entries/${eid}`);
  if(!result) return;
  const idx=DATA.products.findIndex(x=>x.id===pid);
  if(idx>=0) DATA.products[idx]=result;
  renderDetailModal(pid); showToast('Entrada eliminada');
}
...
function productForm(p,defCat){
  ...
  return `<div class="form-grid">
    ...
    <div class="form-group"><label class="form-label">CATEGORÍA</label><select class="form-select" id="np-cat">${cats}</select></div>
    <div class="form-group"><label class="form-label">UNIDAD</label><select class="form-select" id="np-unit">${units}</select></div>
    <div class="form-group"><label class="form-label">TASA DIARIA EST.</label><input class="form-input" type="number" id="np-rate" placeholder="opcional" value="${p?.dailyRate||''}" step="any" min="0"></div>
    <div class="form-group"><label class="form-label">PRECIO</label><input class="form-input" type="number" id="np-price" placeholder="opcional" value="${p?.price||''}" step="any" min="0"></div>
  </div>
  <div class="form-actions">
...
async function saveProduct(pid){
  ...
  let result;
  if(pid){
    result = await api('PUT', `/products/${pid}`, {name,category:cat,unit,dailyRate:rate,price});
  } else {
    result = await api('POST', '/products', {name,category:cat,unit,dailyRate:rate,price});
    if(result) DATA.products.push(result);
  }
  if(result){
    const idx=DATA.products.findIndex(x=>x.id===(pid||result.id));
    if(idx>=0) DATA.products[idx]=result;
  }
  saveProduct._last=result;
  closeModal(); render(); showToast(`${name} ${pid?'actualizado':'creado'} ✓`);
}
...
async function confirmDelete(pid){
  ...
  closeModal(); render(); showToast(`${p.name} eliminado`);
}
...
function quickClose(pid){
  const p=DATA.products.find(x=>x.id===pid);
  const today=new Date().toISOString().split('T')[0];
  document.getElementById('modal-title').textContent='¿Cuándo se terminó?';
  document.getElementById('modal-body').innerHTML=`
    <p style="font-size:13px;color:var(--text-muted);margin-bottom:18px">
      Registra la fecha en que se acabó <strong>${p.name}</strong>.
    </p>
...
async function confirmQuickClose(pid){
  const d=document.getElementById('qc-date').value;
  if(!d){showToast('Selecciona una fecha');return;}
  const result = await api('PATCH', `/products/${pid}/close`, {closed:d});
  if(!result) return;
  const idx=DATA.products.findIndex(x=>x.id===pid);
  if(idx>=0) DATA.products[idx]=result;
  const p=DATA.products.find(x=>x.id===pid);
  closeModal(); render(); showToast(`${p?.name||'Producto'} marcado como terminado ✓`);
}

// ══════════════════════════════════════════════
// MERCADO
// ══════════════════════════════════════════════
let mercadoItems=[], mercadoResults=null;
...
function renderMercado(){
  ...
  return `
    <div class="topbar">
      <div class="topbar-title"><h2>Registrar Mercado</h2><p>¿Qué compraste hoy?</p></div>
    </div>
    <div class="mercado-hero"><h3>🛒 Nuevo mercado</h3><p>Selecciona cada producto, su cantidad y precio.</p></div>
    <div class="mercado-form">
      ...
      <div class="mercado-row-head">
        <span>Producto</span><span class="price-head">Precio ($)</span><span>Cantidad</span><span></span>
      </div>
...
async function procesarMercado(){
  const date_val=document.getElementById('mercado-date').value;
  ...
  const result = await api('POST', '/mercado', {date:date_val, items:valid});
  if(!result) return;
  mercadoResults=valid;
  await loadProducts();
  render();
...
function renderMercadoResults(){
  ...
  return `<div class="prediction-card">
    <h4>📊 Proyección · próximos ${next} días</h4>
    ${mercadoResults.map(it=>{
      const p=DATA.products.find(x=>x.id===it.pid); if(!p) return '';
      const rate=recalcRate(p),d=getDaysLeft(p),s=getStatus(p);
      return `<div class="pred-item">
        <div><div class="pred-name">${p.name}</div><div class="pred-detail">${rate?fmtN(rate,p.unit)+' '+p.unit+'/día · ':''}${fmtN(it.qty,p.unit)} ${p.unit} comprados</div></div>
        <div class="pred-days"><strong style="color:${s==='critical'?'var(--red)':s==='warning'?'var(--amber)':'var(--teal)'}">${d!==null?Math.round(d):'?'}</strong><span>días est.</span></div>
  ...
  <div style="background:var(--teal-light);border-radius:var(--radius);padding:14px 18px;border:1.5px solid var(--teal)">
    <div style="font-weight:600;color:var(--teal-dark);margin-bottom:8px">🗓️ Comprar en próximo mercado</div>
    <div style="display:flex;flex-wrap:wrap;gap:7px">
      ${needRestock.length?needRestock.map(it=>{const p=DATA.products.find(x=>x.id===it.pid);return `<span style="background:var(--teal);color:#fff;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:500">${p.name}</span>`;}).join(''):'<span style="color:var(--teal-dark);font-size:13px">¡Todo alcanza para el próximo mercado! 🎉</span>'}
    </div>
  </div>`;
}
  ...
  CATEGORIES.forEach(cat=>{
    ...
    ps.forEach(p=>{
      ...
      html+=`<button class="picker-product-btn ${sel?'selected':''}" onclick="selectPickerProduct('${p.id}')">
        <div><div style="font-weight:${sel?600:400}">${p.name} ${sel?'✓':''}</div>
        <div class="picker-product-meta">${rate?fmtN(rate,p.unit)+' '+p.unit+'/día':''}</div></div>
        ${p.price?`<span style="font-size:11.5px;color:var(--text-muted)">$${p.price.toLocaleString('es-CO')}</span>`:''}
      </button>`;
    });
  });
  if(!html) html=`<div style="text-align:center;padding:20px;color:var(--text-muted);font-size:13px">No se encontró "${q}"</div>`;
...
function selectPickerProduct(pid){ if(pickerItemIndex>=0) mercadoItems[pickerItemIndex].pid=pid; closePicker(); render(); }
...
async function saveProductAndPick(){
  const name=document.getElementById('np-name').value.trim();
  const cat=document.getElementById('np-cat').value;
  const unit=document.getElementById('np-unit').value;
  const rate=parseFloat(document.getElementById('np-rate').value)||null;
  const price=parseFloat(document.getElementById('np-price').value)||null;
  if(!name){showToast('El nombre es obligatorio');return;}
  const result = await api('POST', '/products', {name,category:cat,unit,dailyRate:rate,price});
  if(!result) return;
  DATA.products.push(result);
  mercadoItems[pickerItemIndex].pid=result.id;
...
function renderGraficas(){
  ...
  return `
    <div class="topbar"><div class="topbar-title"><h2>Gráficas</h2><p>Consumo histórico desde el inicio del registro</p></div></div>
    <div class="charts-grid">
      <div class="chart-card full">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:10px">
          <h4 style="margin:0" id="gc-title">📈 Consumo mensual</h4>
          <select class="form-select" id="gc-select" style="max-width:220px;font-size:13px" onchange="gcDrawConsumo()">
            <option value="">— Selecciona un producto —</option>${opts}
          </select>
        </div>
        <canvas id="gc-consumo"></canvas>
      </div>
      <div class="chart-card"><h4>🧂 Tasa diaria — Sólidos (g/día)</h4><canvas id="gc-rate-solid"></canvas></div>
      <div class="chart-card"><h4>💧 Tasa diaria — Líquidos (ml/día)</h4><canvas id="gc-rate-liquid"></canvas></div>
      <div class="chart-card"><h4>⏱ Cocina — días por presentación</h4><canvas id="gc-dur-Cocina"></canvas></div>
      <div class="chart-card"><h4>⏱ Uso personal — días por presentación</h4><canvas id="gc-dur-Uso personal"></canvas></div>
      <div class="chart-card"><h4>⏱ Aseo — días por presentación</h4><canvas id="gc-dur-Aseo"></canvas></div>
      <div class="chart-card"><h4>⏱ Bebeca — días por presentación</h4><canvas id="gc-dur-Bebeca"></canvas></div>
    </div>`;
}

let _chartsData = null;
async function drawGraficas(){
  _chartsData = await api('GET', '/charts');
  if(!_chartsData) return;
  // Auto-select first product
  const sel=document.getElementById('gc-select');
  if(sel&&sel.options.length>1){ sel.selectedIndex=1; gcDrawConsumo(); }
  gcDrawRates();
  gcDrawDurations();
}

function gcDrawConsumo(){
  if(!_chartsData) return;
  const sel=document.getElementById('gc-select');
  const pid=sel?sel.value:''; if(!pid) return;
  const pd=_chartsData.monthly[pid]; if(!pd) return;
  const title=document.getElementById('gc-title');
  if(title) title.textContent=`📈 ${pd.name} — consumo mensual (${pd.unit})`;
  const col=CAT_COLOR[pd.category]||'#1A7B6E';
  gcMake('gc-consumo',{type:'bar',
    data:{labels:_chartsData.monthLabels,datasets:[{label:pd.unit+'/mes',data:pd.data,
    ...
    options:{responsive:true,plugins:{legend:{display:false}},
      scales:{x:{grid:{display:false}},y:{beginAtZero:true,grid:{color:'#E2DDD5'},
        title:{display:true,text:pd.unit+' / mes'}}}}
  });
}

function gcDrawRates(){
  if(!_chartsData) return;
  const rates=_chartsData.rates;
  function rateChart(id,unitType,lbl){
    const prods=rates.filter(r=>r.unitType===unitType).slice(0,10);
    if(!prods.length) return;
    gcMake(id,{type:'bar',
      data:{labels:prods.map(r=>r.name),datasets:[{label:lbl,data:prods.map(r=>r.rate),
        backgroundColor:prods.map(r=>CAT_COLOR[r.category]+'BB'),
        borderColor:prods.map(r=>CAT_COLOR[r.category]),borderWidth:1.5,borderRadius:5}]},
      options:{indexAxis:'y',responsive:true,plugins:{legend:{display:false}},
        scales:{x:{beginAtZero:true,grid:{color:'#E2DDD5'},title:{display:true,text:lbl}},
          y:{grid:{display:false},ticks:{font:{size:11}}}}}
    });
  }
  rateChart('gc-rate-solid','solid','g/día');
  rateChart('gc-rate-liquid','liquid','ml/día');
}

function gcDrawDurations(){
  if(!_chartsData) return;
  CATEGORIES.forEach(cat=>{
    const id='gc-dur-'+cat;
    const prods=Object.values(_chartsData.durations).filter(d=>d.category===cat).sort((a,b)=>b.avg-a.avg);
    if(!prods.length) return;
    const col=CAT_COLOR[cat]||'#6B7280';
    gcMake(id,{type:'bar',
      data:{labels:prods.map(d=>d.name),datasets:[{label:'días',data:prods.map(d=>d.avg),
        backgroundColor:col+'BB',borderColor:col,borderWidth:1.5,borderRadius:5}]},
      options:{indexAxis:'y',responsive:true,plugins:{legend:{display:false},
        tooltip:{callbacks:{label:ctx=>' '+ctx.parsed.x+' días'}}},
        scales:{x:{beginAtZero:true,grid:{color:'#E2DDD5'},title:{display:true,text:'días'}},
          y:{grid:{display:false},ticks:{font:{size:11}}}}}
    });
  });
}
...
// ══════════════════════════════════════════════
render().then(() => {
  // Carga la lista de mercado después del render inicial
  if(currentView==='dashboard') loadListaMercado();
});
</script>
</body>
</html>
